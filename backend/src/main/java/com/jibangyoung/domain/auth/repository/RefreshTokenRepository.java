...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, Long> {
    ...
    @Modifying
    @Query("DELETE FROM RefreshToken rt WHERE rt.user = :user AND (rt.revoked = true OR rt.expiresAt < :now)")
    void deleteUserExpiredAndRevokedTokens(@Param("user") User user, @Param("now") LocalDateTime now);
